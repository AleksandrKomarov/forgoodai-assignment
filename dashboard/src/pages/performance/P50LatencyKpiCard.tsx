...
export default function P50LatencyKpiCard() {
        ...
        {exceeded ? (
          ...
          <>
            <div className="kpi-value">
              {data && formatDuration(data.p50_ms)}
            </div>
            <div className={`kpi-delta ${data ? getLatencyDeltaClass(data.delta_p50_ms) : ""}`}>
              {data && formatLatencyDelta(data.delta_p50_ms)}
            </div>
          </>
        )}
      </div>
