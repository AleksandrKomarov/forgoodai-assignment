...
export default function P95LatencyKpiCard() {
        ...
        {exceeded ? (
          ...
          <>
            <div className="kpi-value">
              {data && formatDuration(data.p95_ms)}
            </div>
            <div className={`kpi-delta ${data ? getLatencyDeltaClass(data.delta_p95_ms) : ""}`}>
              {data && formatLatencyDelta(data.delta_p95_ms)}
            </div>
          </>
        )}
      </div>
